...
# Uploads runs.json and PNGs to runs/mesoensprob/refs/storm_coverage/
# ============================================================
...
DATA_DIR = os.path.join(BASE_DIR, "refs_storm_coverage_subsets")
...
def find_latest_available_refs_cycle(max_back_hours=48):
    ...
    for back in range(max_back_hours + 1):
        ...
        if dt.hour not in VALID_REFS_CYCLES:
            continue
...
def load_refs_fields_once(fhr):

    ltng_da = refs_idx_field(
        init_dt,
        fhr,
        [
            ["LTNG", "prob >0.2"],
...
def plot_domain_from_fields(fields, domain_key, cfg, fhr):
    ...
    outname = os.path.join(domain_outdir, f"refs_storm_coverage_f{fhr:03d}.png")
...
for fhr in fhrs:
    ...
    try:
        fields = load_refs_fields_once(fhr)
...
print("Done. Uploaded REFS storm coverage probability to R2:", R2_PRODUCT_PATH)
