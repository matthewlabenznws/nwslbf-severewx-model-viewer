...
# Storm Coverage
# Fill: Lightning probability > 0.2
# Contours: Composite Reflectivity > 40 dBZ probability
# Uploads runs.json and PNGs to runs/mesoensprob/href/storm_coverage/
# ============================================================
...
DATA_DIR = os.path.join(BASE_DIR, "href_storm_coverage_subsets")

SECTION_KEY = "mesoensprob"
MODEL_KEY = "href"
PRODUCT_KEY = "storm_coverage"
...
def load_href_fields_once(fhr):
    ...
    ltng_da = href_idx_field(
        init_dt,
        fhr,
        [
            ["LTNG", "prob >0.2"],
            ["LTNG", "surface", "prob >0.2"],
            ["LTNG"],
        ],
        "Lightning >0.2 probability"
    )

    lat, lon = get_lat_lon(ltng_da)

    ltng_prob = normalize_probability(
        ensure_2d_field(ltng_da, "Lightning >0.2 probability")
    )

    refc40_da = href_idx_field(
        init_dt,
        fhr,
        [
            ["REFC", "prob >40"],
    ...
    refc40_prob = normalize_probability(
        ensure_2d_field(refc40_da, "REFC >40 probability")
    )
...
def plot_domain_from_fields(fields, domain_key, cfg, fhr):
    ...
    try:
        lat = fields["lat"]
        lon = fields["lon"]
        ltng_prob = fields["ltng_prob"]
        refc40_prob = fields["refc40_prob"]
        ...
        ltng_plot = gaussian_filter(np.nan_to_num(ltng_sub, nan=0.0), sigma=0.7)
        refc40_plot = gaussian_filter(np.nan_to_num(refc40_sub, nan=0.0), sigma=0.7)

        ltng_plot = np.where(ltng_plot >= 5, ltng_plot, np.nan)
        refc40_plot = np.where(refc40_plot >= 5, refc40_plot, np.nan)
        ...
        if np.isfinite(refc40_plot).any() and np.nanmax(refc40_plot) >= 10:
            cs = ax.contour(
                lon_sub,
                lat_sub,
                refc40_plot,
        ...
        outname = os.path.join(domain_outdir, f"href_storm_coverage_f{fhr:02d}.png")
...
print("Done. Uploaded HREF storm coverage probability to R2:", R2_PRODUCT_PATH)
