# ============================================================
# HRRR Max 0-2 km UH Swath | R2 / CAMs Site Version
...
UH_cmap = ListedColormap(UH_colors, name="UH_bins")
UH_norm = BoundaryNorm(UH_bounds, UH_cmap.N, clip=True)
...
# ============================================================
# LOAD 0-2 KM UH ONCE PER FORECAST HOUR
# ============================================================

def load_uh_once(fhr):
    print("\n" + "=" * 70)
    print(f"Loading HRRR max 0-2 km UH | {cycle_date} {cycle_hour:02d}Z F{fhr:03d}")
    print("=" * 70)
    ...
    for search in search_options:
        try:
            uh_da = hrrr_field(
                ...
                product="sfc",
                search=search,
                label=f"0-2 km UH {search}"
            )

            lat, lon = get_lat_lon(uh_da)
            uh = np.asarray(uh_da.values, dtype=float)

            finite_max = np.nanmax(uh)
            print(f"Raw 0-2 km UH max: {finite_max:.2f}")

            uh = np.where(uh >= 2.5, uh, np.nan)
    ...
    raise RuntimeError(f"Could not open HRRR 0-2 km UH field. Last error: {last_err}")
...
def plot_uh_domain(fields, domain_key, cfg, fhr):
    extent = cfg["extent"]
    ...
    lat_sub, lon_sub, [uh_sub] = subset_2d(
        lat,
        lon,
        extent,
        uh
    ...
    uh_plot = gaussian_filter(
        np.nan_to_num(uh_sub, nan=0.0),
        sigma=1.0
    )

    uh_plot = np.where(uh_plot >= 2.5, uh_plot, np.nan)
    ...
    main_title = "HRRR | Maximum 0-2 km Updraft Helicity Swath"
    ...
    cbar.set_label("0-2 km Updraft Helicity (m² s⁻²)", fontsize=10, weight="bold")
    ...
    outname = os.path.join(
        domain_outdir,
        f"hrrr_max_uh_02_f{fhr:03d}.png"
    )
...
for fhr in fhrs:
    fields = load_uh_once(fhr)

    if running_uh is None:
        running_uh = fields["uh"].copy()
        base_lat = fields["lat"]
        base_lon = fields["lon"]
        uh_search_used = fields.get("search", "")
    else:
        running_uh = np.fmax(running_uh, fields["uh"])

    swath_fields = {
        "lat": base_lat,
        "lon": base_lon,
        "uh": running_uh,
        "search": uh_search_used,
    }

    for domain_key, cfg in DOMAINS.items():
        plot_uh_domain(swath_fields, domain_key, cfg, fhr)

print("Done. Uploaded HRRR max 0-2 km UH swath to R2:", R2_PRODUCT_PATH)
