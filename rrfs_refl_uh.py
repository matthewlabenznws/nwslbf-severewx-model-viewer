# ============================================================
# RRFS_A | R2 CAMs Product
# Reflectivity + UH + Sim IR + Theta Cold Pools + 4–6 km SR Winds
# Uploads runs.json and PNGs to runs/cams/rrfs/refl_uh/
# ============================================================
...
def rrfs_grib_url(init_dt, fhr, product="2dfld"):
    ...
    if product == "2dfld":
        ...
        fname = f"rrfs.t{hh}z.prslev.3km.f{fhr:03d}.conus.grib2"
    else:
...
def upload_runs_json(init_dt, cycle_str, max_fhr):
    ...
    for r in old_runs:
        if isinstance(r, str):
            ...
            combined.append({
                "id": rid,
                "label": rid.replace("_", " "),
                "max_fhr": 60 if rhour in LONG_CYCLE_HOURS else 18,
            })
    ...
    print("Uploaded runs.json with last 4 RRFS runs.")
...
def plot_domain_from_fields(fields, domain_key, cfg, fhr):
    ...
    try:
        ...
        outname = os.path.join(domain_outdir, f"rrfs_lbf_f{fhr:03d}.png")
...
print("Done. Uploaded RRFS reflectivity/UH to R2:", R2_PRODUCT_PATH)