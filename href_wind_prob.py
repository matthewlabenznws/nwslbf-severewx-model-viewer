...
# Uploads PNGs to runs/mesoensprob/href/wind_prob/
# ============================================================
...
DATA_DIR = os.path.join(BASE_DIR, "href_wind_prob_subsets")
SECTION_KEY = "mesoensprob"
MODEL_KEY = "href"
PRODUCT_KEY = "wind_prob"
R2_PRODUCT_PATH = f"runs/{SECTION_KEY}/{MODEL_KEY}/{PRODUCT_KEY}"
OUTDIR_BASE = os.path.join("site", "runs", SECTION_KEY, MODEL_KEY, PRODUCT_KEY)
...
def upload_to_r2(local_file, remote_key, content_type="image/png"):
    s3.upload_file(local_file, BUCKET, remote_key, ExtraArgs={"ContentType": content_type})
    print("Uploaded to R2:", remote_key)

DOMAINS = {
    "lbf": {"label": "LBF", "extent": [-103.8, -97.0, 40.0, 43.4], "title_size": 12, "subtitle_size": 11},
    "regional": {"label": "Default", "extent": [-107.5, -93.0, 38.5, 44.2], "title_size": 13, "subtitle_size": 11},
    "central_plains": {"label": "Central Plains", "extent": [-107.5, -91.0, 34.5, 45.2], "title_size": 13, "subtitle_size": 11},
}

SPC_DAY1_CAT_URL = "https://mapservices.weather.noaa.gov/vector/rest/services/outlooks/SPC_wx_outlks/MapServer/1/query"
SPC_RISK_ORDER = {"TSTM": 1, "MRGL": 2, "SLGT": 3, "ENH": 4, "MDT": 5, "HIGH": 6}
MIN_SPC_RISK = "SLGT"
...
def add_spc_severe_domain():
    try:
        params = {"where": "1=1", "outFields": "*", "f": "geojson", "returnGeometry": "true", "outSR": "4326"}
        ...
        centroid_ll = gpd.GeoSeries(main_gdf.to_crs(epsg=5070).geometry.centroid, crs="EPSG:5070").to_crs(epsg=4326).iloc[0]
...
def ensure_2d_field(da, label):
    ...
    if arr.ndim != 2:
        raise RuntimeError(f"{label} is not 2D after squeeze. Shape={arr.shape}")
    return arr
...
def add_shapefile_outline(ax, shp_path, edgecolor="k", linewidth=1.2, zorder=6):
    ...
    ax.add_geometries(gdf.geometry, crs=ccrs.PlateCarree(), facecolor="none", edgecolor=edgecolor, linewidth=linewidth, zorder=zorder)

def get_lbf_cwa_geom(cwa_shp_path):
    if not os.path.exists(cwa_shp_path):
        return None
    recs = list(shpreader.Reader(cwa_shp_path).records())
    geoms = [r.geometry for r in recs if str(r.attributes.get("CWA", "")).upper() == "LBF" or str(r.attributes.get("WFO", "")).upper() == "LBF"]
    if not geoms:
...
def add_counties_clipped_to_cwa(ax, counties_shp_path, cwa_geom, lw=1.0, color="black", zorder=6):
    ...
    ax.add_geometries(clipped, crs=ccrs.PlateCarree(), facecolor="none", edgecolor=color, linewidth=lw, zorder=zorder)

def grib_url(init_dt, fhr):
    ymd = init_dt.strftime("%Y%m%d")
    hh = init_dt.strftime("%H")
    fname = f"href.t{hh}z.conus.prob.f{fhr:02d}.grib2"
    return f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/href/prod/href.{ymd}/ensprod/{fname}"
...
def find_latest_cycle(max_back_hours=96):
    ...
    for back in range(max_back_hours + 1):
        ...
        if url_exists(grib_url(dt, 1) + ".idx"):
...
def parse_idx_lines(lines):
    ...
    for i, line in enumerate(lines):
        ...
        if len(parts) < 5:
            continue
        try:
            parsed.append({"i": i, "line": line, "msg_num": int(parts[0]), "start": int(parts[1])})
        except Exception:
            continue
    for j in range(len(parsed)):
        parsed[j]["end"] = parsed[j + 1]["start"] - 1 if j < len(parsed) - 1 else None
    return parsed

def find_idx_match(parsed, all_terms, label):
    terms = [t.lower() for t in all_terms]
    matches = [p for p in parsed if all(t in p["line"].lower() for t in terms)]
    if not matches:
        sample = "\n".join([p["line"] for p in parsed[:200]])
        raise RuntimeError(f"Could not find {label} using {all_terms}.\nFirst 200 IDX lines:\n{sample}")
    print(f"Matched {label}:", matches[0]["line"])
    return matches[0]
...
def open_subset_grib(path, label):
    ...
    print(f"Opened {label}: var={var}, shape={ds[var].shape}")
    return ds[var]

def idx_field(init_dt, fhr, term_sets, label):
    g_url = grib_url(init_dt, fhr)
    parsed = parse_idx_lines(read_idx(g_url + ".idx"))
    last_error = None
    for terms in term_sets:
        try:
            match = find_idx_match(parsed, terms, label)
            safe_label = re.sub(r"[^A-Za-z0-9]+", "_", label).strip("_")
            outpath = os.path.join(DATA_DIR, f"href_prob_{init_dt:%Y%m%d_%H}z_f{fhr:02d}_{safe_label}_{match['msg_num']}.grib2")
            download_byte_range(g_url, match["start"], match["end"], outpath)
...
def subset_2d(lat, lon, *fields):
    mask = np.isfinite(lat) & np.isfinite(lon) & (lon >= LON_MIN) & (lon <= LON_MAX) & (lat >= LAT_MIN) & (lat <= LAT_MAX)
    if not np.any(mask):
        raise RuntimeError("No grid points found inside selected domain.")
    iy, ix = np.where(mask)
    iy0, iy1 = max(iy.min() - 2, 0), min(iy.max() + 3, lat.shape[0])
    ix0, ix1 = max(ix.min() - 2, 0), min(ix.max() + 3, lon.shape[1])
    return lat[iy0:iy1, ix0:ix1], lon[iy0:iy1, ix0:ix1], [f[iy0:iy1, ix0:ix1] for f in fields]

def upload_runs_json(init_dt, cycle_str, max_fhr):
    old_runs = []
    try:
        obj = s3.get_object(Bucket=BUCKET, Key=f"{R2_PRODUCT_PATH}/runs.json")
        old_runs = json.loads(obj["Body"].read().decode("utf-8")).get("runs", [])
    except Exception:
        pass
    new_run = {"id": cycle_str, "label": init_dt.strftime("%Y-%m-%d %Hz"), "max_fhr": max_fhr}
    combined = [new_run] + [r for r in old_runs if not (isinstance(r, dict) and r.get("id") == cycle_str)]
    with open("runs.json", "w") as f:
        json.dump({"runs": combined[:4]}, f, indent=2)
    upload_to_r2("runs.json", f"{R2_PRODUCT_PATH}/runs.json", content_type="application/json")
...
def load_fields_once(fhr):
    ...
    wind_prob = normalize_probability(ensure_2d_field(wind_da, "10m wind >25.72 probability"))
    ...
    refc_prob = normalize_probability(ensure_2d_field(refc_da, "REFC >40 probability"))

    return {"lat": lat, "lon": lon, "wind_prob": wind_prob, "refc_prob": refc_prob}
...
def plot_domain(fields, domain_key, cfg, fhr):
    ...
    lat_sub, lon_sub, [wind_sub, refc_sub] = subset_2d(
        fields["lat"], fields["lon"], fields["wind_prob"], fields["refc_prob"]
    )
    ...
    if np.isfinite(refc_plot).any() and np.nanmax(refc_plot) >= 10:
        ...
        ax.clabel(cs, fmt="%d", fontsize=7, inline=True, inline_spacing=3)
    ...
    if lbf_geom is not None:
        ...
        ax.add_geometries([lbf_geom], crs=ccrs.PlateCarree(), facecolor="none", edgecolor="black", linewidth=3.5, zorder=14)
        ax.add_geometries([lbf_geom], crs=ccrs.PlateCarree(), facecolor="none", edgecolor="white", linewidth=1.8, zorder=15)
    ...
    ax.text(1.0, 1.005, f"Init: {init_dt:%a %Y-%m-%d %HZ} {MODEL_LABEL}",
            transform=ax.transAxes, ha="right", va="bottom", fontsize=cfg["subtitle_size"], fontweight="bold")
    ...
    if os.path.exists(LOGO_PATH):
        ...
        logo_ax = ax.inset_axes([0.82, 0.84, 0.165, 0.155], transform=ax.transAxes, zorder=50)
        logo_ax.imshow(logo)
        logo_ax.axis("off")

    ax.text(0.902, 0.835, "NWS North Platte, NE", transform=ax.transAxes,
            ha="center", va="top", fontsize=10, fontweight="bold", color="black",
            zorder=51, path_effects=[pe.withStroke(linewidth=2.5, foreground="white")])

    ax.text(0.01, 0.015, "Plot created by: Matthew Labenz", transform=ax.transAxes,
            ha="left", va="bottom", fontsize=8, weight="bold", color="black",
            zorder=40, path_effects=[pe.withStroke(linewidth=2.5, foreground="white")])

    outname = os.path.join(domain_outdir, f"href_wind_prob_f{fhr:02d}.png")
    plt.savefig(outname, dpi=140, bbox_inches="tight")
    plt.close(fig)

    remote_key = f"{R2_PRODUCT_PATH}/{cycle_str}/{domain_key}/{os.path.basename(outname)}"
...
print("Done. Uploaded HREF wind probability to R2:", R2_PRODUCT_PATH)
