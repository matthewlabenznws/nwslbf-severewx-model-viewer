...
# ============================================================
# RRFS WIND GUST + DCAPE + 10-M WIND
#
# SITE PRODUCT
#
# FILLED:
#   RRFS Surface Wind Gust (mph)
#
# CONTOURS:
...
#
# OUTPUT:
#   runs/cams/rrfs/gust_dcape/
#
# DATA ACCESS:
#   NOMADS .idx inventory + HTTP byte-range requests.
#   This avoids downloading full RRFS CONUS GRIB2 files.
# ============================================================
...
# RRFS ~3 km.
# 6 = roughly 18 km DCAPE sampling.
DCAPE_STRIDE = 6

# Slight smoothing to clean up contours.
...
# Use all RRFS pressure levels from 1000 through 400 mb
# at 25-mb spacing to keep DCAPE close to the HRRR version.
DCAPE_PRESSURE_LEVELS = list(
    range(
        1000,
        399,
        -25
...
# EVERY 100 J/KG
#
# THINNER THAN PREVIOUS VERSION SO THE GUST FIELD
# REMAINS THE DOMINANT FIELD.
# ============================================================
...
# ------------------------------------------------------------
# THINNER DCAPE CONTOURS
#
# Every 500 J/kg remains slightly thicker.
# ------------------------------------------------------------
...
def download_byte_range(
    ...
    raise RuntimeError(
        f"Could not download RRFS "
        f"byte range {range_header}. "
        f"Last error: {last_error}"
    )


# ============================================================
# OPEN ONE RRFS MESSAGE
# ============================================================

def open_single_grib_message(
    path,
    label
):
...
def rrfs_field(
    ...
    try:
        ...
        return open_single_grib_message(
            out_file,
            label
        )
...
# ============================================================
# RRFS PRESSURE PROFILE
#
# Downloads only TMP or DPT messages at requested pressure
# levels and rebuilds a 3-D DataArray for DCAPE.
# ============================================================

def rrfs_pressure_profile(
    init_dt,
    fhr,
    variable,
    levels_hpa,
    ...
    if len(level_arrays) < 8:

        raise RuntimeError(
            f"Only {len(level_arrays)} "
            f"{label} pressure levels "
            "were available; need at least 8."
        )
    ...
    print(
        ...
        f"from {max(valid_levels):.0f} "
        f"to {min(valid_levels):.0f} mb"
    )
...
# ============================================================
# GET LATEST RRFS CYCLE
# ============================================================

init_dt = (
    find_latest_rrfs_cycle()
)

cycle_date = init_dt.strftime(
    "%Y%m%d"
)

cycle_hour = init_dt.hour
...
if cycle_hour in LONG_CYCLE_HOURS:
...
print(
    "Uploaded RRFS gust/DCAPE runs.json."
)
...
def load_rrfs_fields_once(
    ...
    temp_da = rrfs_pressure_profile(
        init_dt,
        fhr,
        "TMP",
        DCAPE_PRESSURE_LEVELS,
        "pressure-level temperature"
    )

    dewpoint_da = rrfs_pressure_profile(
        init_dt,
        fhr,
        "DPT",
        DCAPE_PRESSURE_LEVELS,
...
def plot_domain_from_fields(
    ...
    try:
        ...
        #
        # DRAW THESE BEFORE DCAPE + BARBS SO METEOROLOGICAL
        # FIELDS REMAIN ABOVE THEM.
        # ====================================================
        ...
        # 10-M WIND BARBS
        #
        # ABOVE EVERYTHING EXCEPT TEXT / LABELS
        # ====================================================
        ...
        # MAX GUST BOX
        #
        # NO STAR / NO MAP LOCATION MARKER
        # ====================================================
        ...
        outname = os.path.join(
            domain_outdir,
            f"rrfs_gust_dcape_f{fhr:03d}.png"
        )
...
print(
    "RRFS GUST/DCAPE PROCESSING SUMMARY"
)
